...
class AddRandomTransformationDims(object):
    def __init__(self, angle_set, color_set, scale_set):
        ...
        self.color_set = list(map(lambda x: np.radians(x) - np.pi, color_set))
        self.scale_set = scale_set

    def __call__(self, tensor, transform_type, step):
        x = tensor
        b, c, h, w = x.shape
        ...
        center[..., 0] = w / 2  # x
        center[..., 1] = h / 2  # y
        scale = self.scale_set[step]
        color = self.color_set[step]
        angle = self.angle_set[step]
        bsz_angles = torch.ones(x.shape[0], device=x.device)

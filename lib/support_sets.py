...
class SupportSets(nn.Module):
    ...
    def return_f(self, support_sets_mask, z):
        ...
        f = (alphas_batch * torch.exp(-gammas_batch * (torch.norm(D, dim=2) ** 2).unsqueeze(dim=2))).sum()
        # Return nthe value of f (used for plotting)
        return f
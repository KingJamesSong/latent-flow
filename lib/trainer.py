...
class Trainer(object):
    ...
    def train(self, generator, support_sets, reconstructor):
        ...
        for iteration in range(starting_iter, self.params.max_iter + 1):
            ...
            #target_support_sets_indices = torch.randint(0, self.params.num_support_sets)
            ...
            #energy, shift, loss_wave = support_sets(index,z,time_stamp,generator)
            #energy2, shift2, loss_wave2 = support_sets(index,z,time_stamp2, generator)
            #img_shifted = generator(latent1)
            #img_shifted2 = generator(latent2)
            #if torch.rand(1)>=0.5:
            img_shifted = generator(latent1)
            img_shifted2 = generator(latent2)
                #img_shifted = generator.inference(z+shift)
                #img_shifted2 = generator.inference(z+shift2)
            #else:
            #    img_shifted = generator(2*z - latent2)
            #    img_shifted2 = generator(2*z - latent1)
                #img_shifted = generator.inference(z-shift2)
                #img_shifted2 = generator.inference(z-shift)
            #if torch.rand(1)>=0.5:
            #    img_shifted = generator(z, shift)
            #    img_shifted2 = generator(z, shift2)
            #else:
            #    img_shifted2 = generator(z, -shift2)
            #    img_shifted = generator(z,-shift)
            # Predict support sets indices and shift magnitudes
            predicted_support_sets_indices, predicted_shift_magnitudes = reconstructor(img_shifted,img_shifted2)

            # Calculate classification (cross entropy) and regression (mean absolute error) losses
            #Create one-hot label
            #label = torch.zeros(self.params.batch_size,self.params.num_support_sets)
            #label[:,index]=1
            #print(predicted_support_sets_indices.size(),label.size())
            classification_loss = self.cross_entropy(predicted_support_sets_indices,index.repeat(self.params.batch_size,1).squeeze())
            regression_loss = torch.mean(torch.abs(predicted_shift_magnitudes - time_stamp.detach()))
            #print(predicted_shift_magnitudes.size(),time_stamp.size())
            #regression_loss = self.cross_entropy(predicted_shift_magnitudes,time_stamp.detach().long().squeeze())
            # Calculate total loss and back-propagate
            #loss = self.params.lambda_cls * classification_loss + self.params.lambda_reg * regression_loss + loss_wave
            ...
            self.stat_tracker.update(accuracy_index=torch.mean((torch.argmax(predicted_support_sets_indices, dim=1) ==
                                     ...
                                     accuracy_time=torch.mean((predicted_shift_magnitudes.long() ==
                                                          time_stamp.long()).to(torch.float32)).detach(),
                                     #accuracy_time=torch.mean((torch.argmax(predicted_shift_magnitudes, dim=1) ==
                                     #                           time_stamp.long()).to(torch.float32)).detach(),
